...
# Datasets Options
parser.add_argument(
    "--train_batch_size", default=512, type=int, help="perturb step size"
)
parser.add_argument(
    "--eval_batch_size", default=512, type=int, help="perturb step size"
)
...
parser.add_argument(
    ...
    default="min-min",
    type=str,
    choices=["min-min", "min-max", "random"],
    help="Attack type",
)
parser.add_argument(
    "--perturb_type",
...
parser.add_argument(
    "--noise_shape", default=[10, 3, 32, 32], nargs="+", type=int, help="noise shape"
)
...
parser.add_argument("--noise_name", type=str, default="perturbation")
parser.add_argument("--selected_classes", default=list(range(10)), nargs="+", type=int)
args = parser.parse_args()

print("noise name", args.noise_name)
print("selected classes", args.selected_classes)
...
def train(
    ...
    for epoch in range(starting_epoch, config.epochs):
        ...
        payload = "Eval Loss:%.4f\tEval acc: %.2f" % (
            evaluator.loss_meters.avg,
            evaluator.acc_meters.avg * 100,
        )
        logger.info(payload)
...
def universal_perturbation_eval(
    noise_generator,
    random_noise,
    data_loader,
    model,
    eval_target=args.universal_train_target,
):
    ...
    for i, (images, labels) in enumerate(data_loader[eval_target]):
        ...
        if random_noise is not None:
            for i in range(len(labels)):
                ...
                mask_cord, class_noise = noise_generator._patch_noise_extend_to_img(
                    noise,
...
def universal_perturbation(
    ...
    datasets_generator = dataset.DatasetGenerator(
        ...
        seed=args.seed,
        no_train_augments=True,
    )

    if args.use_subset:
        data_loader = datasets_generator._split_validation_set(
            train_portion=args.universal_train_portion,
            train_shuffle=True,
    ...
    while condition:
        if args.attack_type == "min-min" and not args.load_model:
            ...
            for j in range(0, args.train_step):
                ...
                trainer.train_batch(
                    torch.stack(train_imgs).to(device), labels, model, optimizer
                )

        for i, (images, labels) in tqdm(
            enumerate(data_loader[args.universal_train_target]),
            ...
            for i, (image, label) in enumerate(zip(images, labels)):
                noise = random_noise[label.item()]
                mask_cord, class_noise = noise_generator._patch_noise_extend_to_img(
                    noise, image_size=image.shape, patch_location=args.patch_location
                )
            ...
            if args.attack_type == "min-min":
                ...
                perturb_img, eta = noise_generator.min_max_attack(
                    images,
                    labels,
                    model,
                    optimizer,
            ...
            for key in class_noise_eta:
                delta = (
                    torch.stack(class_noise_eta[key]).mean(dim=0) - random_noise[key]
                )
                class_noise = random_noise[key]
                class_noise += delta
                random_noise[key] = torch.clamp(
                    class_noise, -args.epsilon, args.epsilon
                )

        # Eval termination conditions
        loss_avg, error_rate = universal_perturbation_eval(
            noise_generator,
            random_noise,
            data_loader,
            model,
        ...
        logger.info(
            "Loss: {:.4f} Acc: {:.2f}%".format(loss_avg, 100 - error_rate * 100)
        )
...
def samplewise_perturbation_eval(
    random_noise, data_loader, model, eval_target="train_dataset", mask_cord_list=[]
):
    loss_meter = util.AverageMeter()
    err_meter = util.AverageMeter()
    # random_noise = random_noise.to(device)
...
def sample_wise_perturbation(
    ...
    if args.train_data_type == "ImageNetMini" and args.perturb_type == "samplewise":
        data_loader = datasets_generator._split_validation_set(
            0.2, train_shuffle=False, train_drop_last=False
        )
        data_loader["train_dataset"] = data_loader["train_subset"]
    else:
        data_loader = datasets_generator.getDataLoader(
            train_shuffle=False, train_drop_last=False
        )
    mask_cord_list = []
    idx = 0
    for images, labels in data_loader["train_dataset"]:
        for i, (image, label) in enumerate(zip(images, labels)):
            ...
            mask_cord, _ = noise_generator._patch_noise_extend_to_img(
                noise, image_size=image.shape, patch_location=args.patch_location
            )
            mask_cord_list.append(mask_cord)
            idx += 1
    ...
    while condition:
        ...
        idx = 0
        for i, (images, labels) in tqdm(
            enumerate(data_loader["train_dataset"]),
            total=len(data_loader["train_dataset"]),
        ):
            images, labels, model = (
                images.to(device),
                labels.to(device),
        ...
        # Eval termination conditions
        loss_avg, error_rate = samplewise_perturbation_eval(
            random_noise,
            data_loader,
            model,
...
def main():
    ...
    if args.attack_type == "random":
        ...
        logger.info(
            "Noise saved at %s" % (os.path.join(args.exp_name, "perturbation.pt"))
        )
        ...
        torch.save(noise, os.path.join(args.exp_name, args.noise_name + ".pt"))
        # logger.info(noise)
        logger.info(noise.shape)
        logger.info(
            "Noise saved at %s" % (os.path.join(args.exp_name, args.noise_name + ".pt"))
        )
    else:
